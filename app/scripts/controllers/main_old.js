...
qtoolControllers.controller('AdminCtrl', function ($scope, $window, AuthService, PollService, Themes, $q) {
	...
	$scope.currentTemplate = 'newPoll';
	$scope.newPollAvailable = false;
	$scope.unpublishedPollAvailable = false;
	$scope.showGenericError = false;
	$scope.latestPoll;
	$scope.newPollAvailable = false;
	$scope.genericError = 'There was an error while creating the poll, try again in a minute.';
	var defaultPoll = {
		'question': '', 
		'duration': '01:30', 
		'answers': ['', ''], 
	...
	$scope.switchTemplate = function(template) {
		console.log('changing current template to ' , template)
	...
	$scope.createPoll = function() {
		...
		PollService.savePoll($scope.poll).then(function(data) {
			if(data.success) {
				//$scope.switchTemplate('createdPoll'); REFACTORED
			} else {
...
console.log('HOMO ' , $scope.unpublishedPoll);


	//THIS IS WHERE THE PROBLEM IS, LATESTPOLL != unpublishedPoll



		console.log('publishing poll with id: ' , $scope.latestPoll)
